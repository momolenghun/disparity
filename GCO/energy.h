...
class Energy : Graph<value,totalvalue,totalvalue>
{
public:
	typedef typename Graph<value, totalvalue, totalvalue>::node_id Var;
	typedef value Value;	
	typedef totalvalue TotalValue;	
...
inline Energy<V,T>::Energy(void (*err_function)(const char *)) : Graph<V,T,T>(0,0,err_function)
{
	Econst = 0;
...
template <typename V, typename T>
inline Energy<V,T>::~Energy() {}

template <typename V, typename T>
inline typename Energy<V,T>::Var Energy<V,T>::add_variable() {	return this->add_node(); }

template <typename V, typename T>
inline void Energy<V,T>::add_constant(Value A) { Econst += A; }

template <typename V, typename T>
inline void Energy<V,T>::add_term1(Var x,
                              Value A, Value B)
{
	add_tweights(x, B, A);
}
...
template <typename V, typename T>
inline void Energy<V,T>::add_term2(Var x, Var y,
                              Value A, Value B,
                              Value C, Value D)
{
	/* 
...
template <typename V, typename T>
inline void Energy<V,T>::add_term3(Var x, Var y, Var z,
                              Value E000, Value E001,
                              Value E010, Value E011,
                              Value E100, Value E101,
                              Value E110, Value E111)
...
template <typename V, typename T>
inline typename Energy<V,T>::TotalValue Energy<V,T>::minimize() { return Econst + this->maxflow(); }

template <typename V, typename T>
inline int Energy<V,T>::get_var(Var x) { return (int) what_segment(x); }
